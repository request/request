...
let events = []
let caFile = path.resolve(__dirname, 'ssl/ca/ca.crt')
let ca = fs.readFileSync(caFile)
let clientCert = fs.readFileSync(path.resolve(__dirname, 'ssl/ca/client.crt'))
let clientKey = fs.readFileSync(path.resolve(__dirname, 'ssl/ca/client-enc.key'))
...
let ss = server.createSSLServer(sslOpts)
let ss2 = server.createSSLServer(mutualSSLOpts)
