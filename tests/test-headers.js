...
const hasIPv6interface = Object.keys(interfaces).some((name) => {
  return loopbackKeyTest.test(name) && interfaces[name].some((info) => {
...
tape('strip port in host header if explicit standard port (:80) & protocol (HTTP)', (t) => {
  const r = request({
...
tape('strip port in host header if explicit standard port (:443) & protocol (HTTPS)', (t) => {
  const r = request({
...
tape('strip port in host header if implicit standard port & protocol (HTTP)', (t) => {
  const r = request({
...
tape('strip port in host header if implicit standard port & protocol (HTTPS)', (t) => {
  const r = request({
