...
Redirect.prototype.onResponse = function (response) {
  ...
  if (response.statusCode !== 401 && response.statusCode !== 307) {
    ...
    // reinitialize FormData stream for 307 redirects
    delete request._form
    ...
    var formData = {}
    var formKeys = Object.keys(request.formData)
    ...
    for (var i = 0, ii = formKeys.length; i < ii; i++) {
      var formKey = formKeys[i]
      var formValue = request.formData[formKey]
      if (Array.isArray(formValue)) {
        for (var j = 0, jj = formValue.length; j < jj; j++) {
