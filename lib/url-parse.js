var url = require('url')
var encoder = require('postman-url-encoder/encoder')
var EMPTY = ''
var STRING = 'string'
var AMPERSAND = '&'
var EQUALS = '='
...
// This set contains characters that needs to be encoded in query string
// according to RFC 3986 but not encoded by Node's Url.parse().
var encodeSet = new encoder.EncodeSet([' ', '!', '"', '>', '<', '(', ')', '%', '\'', '*'])
...
stringify = function (parameters) {
  return parameters ? parameters.map(function (param) {
    ...
    if (value === null) {
      return encoder.percentEncode(key, encodeSet)
    }

    return encoder.percentEncode(key, encodeSet) + EQUALS + encoder.percentEncode(value, encodeSet)
  }).join(AMPERSAND) : ''
}
