...
function onPipe(src) {
  ...
  if (!readStream && src.method && !self.explicitMethod) {
    self.method = src.method
  }

  function getMissingHeaders(header) {
    return !self.hasHeader(header)
  }

  function setSrcHeader(header) {
    self.setHeader(header, src.headers[header])
  }

  if(!readStream && src.headers) {
    var headers = Object.getOwnPropertyNames(src.headers)
    headers.filter(getMissingHeaders).forEach(setSrcHeader)
  }
